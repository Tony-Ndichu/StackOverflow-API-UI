...
const makeProfile = (first_name, last_name, username, email, questions, answers, parentId,  elementType) => {
    ...
    .then((res) => {
        res.json().then((data) => {
				...
				for (let i in data.list) {
					makeProfile(data.list[i]['first_name'],data.list[i]['last_name'], data.list[i]['username'], data.list[i]['email'], data.list[i]['no_of_questions'], data.list[i]['no_of_answers'], 'profile', 'div')
				}
